...
const controller = {
  init: function () {
    let passengerSeats = document.getElementById("passengerSeats");
    ...
    document.getElementById("button").addEventListener("click", function () {
      ...
      view.clearFromDOM(passengerSeats); // removing old result
  ...
  isInputValid: function (rowsAndColumns, que) {
    if (rowsAndColumns.length > 4) {
    ...
    for (i = 0; i < rowsAndColumns.length; i++) {
      for (j = 0; j < rowsAndColumns[i].length; j++) {
        if (rowsAndColumns[i][j] < 1 || Number.isNaN(rowsAndColumns[i][j])) {
...
const view = {
  ...
  addBackgroundImg: function () {
    passengerSeats.style.background = "url('img/img.png') no-repeat center top";
    passengerSeats.style.backgroundSize = "contain";
  },
  ...
  createTableResults: function (arrInput, arrResult) {
    for (i = 0; i < arrInput.length; i++) {
      ...
      passengerSeats.appendChild(table);
    }
  },
};
